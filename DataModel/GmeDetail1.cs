using System;
using System.Collections.Generic;

namespace DataModel
{
    public partial class GmeDetail1
    {
        public string? TransactionNumber { get; set; }
        public string? TransactionItem { get; set; }
        public string? TransactionType { get; set; }
        public string? TransactionDate { get; set; }
        public string? SoNumber { get; set; }
        public string? SoItem { get; set; }
        public string? SoType { get; set; }
        public string? SoDate { get; set; }
        public string? PoType { get; set; }
        public string? Material { get; set; }
        public string? SerialNumber { get; set; }
        public string? ListPrice { get; set; }
        public string? NetPrice { get; set; }
        public string? CustomerGroup { get; set; }
        public string? EndCustName { get; set; }
        public string? EndCustCountryCode { get; set; }
        public string? EndCustCountry { get; set; }
        public string? SameEndCust { get; set; }
        public string? ResellerName { get; set; }
        public string? ResellerCountryCode { get; set; }
        public string? ResellerCountry { get; set; }
        public string? ContractNumber { get; set; }
        public string? ContractType { get; set; }
        public string? ContractItemNo { get; set; }
        public string? ContractCreationDate { get; set; }
        public string? ContractStatus { get; set; }
        public string? ContractLineStartDate { get; set; }
        public string? ContractLineEndDate { get; set; }
        public string? SvcEndCustName { get; set; }
        public string? SvcEndCustCountryCode { get; set; }
        public string? SvcEndCustCountry { get; set; }
        public string? SvcMaterialKey { get; set; }
        public string? ContractLineItemStatus { get; set; }
        public string? AccountManager { get; set; }
        public string? AccountManagerCountryCode { get; set; }
        public string? AccountManagerCountry { get; set; }
        public string? DistiKey { get; set; }
        public string? DistiName { get; set; }
        public string? DistiCountry { get; set; }
        public string? DistiCountryCode { get; set; }
        public string? DistiTheater { get; set; }
        public string? SoDateNew { get; set; }
        public string? EcGblGroupCompare { get; set; }
        public double? EcGblJaroScore { get; set; }
        public string? SvcGblGroupCompare { get; set; }
        public double? SvcGblJaroScore { get; set; }
        public string? DiversionType { get; set; }
        public string? PartnerLevelKey { get; set; }
        public string? Attribute1 { get; set; }
        public string? ResGblGroupCompare { get; set; }
        public double? ResGblJaroScore { get; set; }
        public int? ResellerMasterId { get; set; }
        public string? ResellerEntityKey { get; set; }
        public int? EndCustMasterId { get; set; }
        public string? EndCustEntityKey { get; set; }
        public int? SvcEcMasterId { get; set; }
        public string? SvcEcEntityKey { get; set; }
    }
}
